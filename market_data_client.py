...
async def _csv_close_all():
    async with _csv_lock:
        ...
        _csv_writers.clear(); _csv_files.clear()
...
def _instrument_short(code: int) -> str:
    try:
        if code == pb.Instrument.INSTRUMENT_SPOT:       return "spot"
        if code == pb.Instrument.INSTRUMENT_PERPETUAL:  return "perpetual"
        if code == pb.Instrument.INSTRUMENT_SWAP:       return "swap"
...
class Tick:
    bid: float; ask: float; mid: float; ts_ns: int; instrument: str
    latency_ms: Optional[float] = None

@dataclasses.dataclass
class Funding:
    mark_price: float; index_price: float; est_settle_price: float
    funding_rate: float; next_funding_time_ms: int; ts_ns: int
    latency_ms: Optional[float] = None

@dataclasses.dataclass
class Fee:
    maker_rate: float; taker_rate: float; ts_ns: int
    latency_ms: Optional[float] = None

@dataclasses.dataclass
class Volume:
    volume0: float; volume1: float; high: float; low: float; trades: int; ts_ns: int
    latency_ms: Optional[float] = None

@dataclasses.dataclass
class Slippage:
    impact_bps01: float; impact_bps10: float; block_number: int; tx_hash: str; ts_ns: int
    latency_ms: Optional[float] = None

@dataclasses.dataclass
class DexPair:
    exchange: str; chain: str; pair: str
    token0: str; token1: str; price01: float; price10: float; invert_for_quote: bool; ts_ns: int
    latency_ms: Optional[float] = None 
...
class CexConfig:
    ...
    symbols: List[str]             # e.g., ["ETHUSDT","BTCUSDT"]
    instruments: List[str]         # e.g., ["spot","perpetual"]
    want: Iterable[str] = ("tick","funding","fee","volume")  # funding은 perp만 자동 적용

@dataclasses.dataclass
class DexConfig:
    exchange: str                  # e.g., "PancakeSwapV2"
    chain: str                     # e.g., "BSC"
    pairs: List[str]               # e.g., ["WBNBUSDT","WBNBETH", ...]
    want: Iterable[str] = ("tick","slippage","fee","volume") # instrument는 항상 swap
...
class MarketDataClient:
    ...
    def __init__(
        ...
        stream: Optional[str] = "MD",            # use_jetstream=True일 때 권장
        ...
        # caches
        self.ticks: Dict[Tuple[str,str,str], Tick] = {}        # (exchange, symbol, instrument)
        self.funding: Dict[Tuple[str,str], Funding] = {}       # (exchange, symbol)
        self.fees: Dict[Tuple[str,str,str], Fee] = {}          # (exchange, symbol, instrument)
        self.volume: Dict[Tuple[str,str,str], Volume] = {}     # (exchange, symbol, instrument)
        self.slip: Dict[Tuple[str,str], Slippage] = {}         # (exchange, pair or symbol)
        self.dex_pairs: Dict[Tuple[str,str,str], DexPair] = {} # (exchange, chain, pair)
            ...
            if self.stream:
                try:
                    ...
                    await self.js.add_stream(jsapi.StreamConfig(
                        ...
                        max_msgs=-1, max_bytes=-1, max_age=0,
                        num_replicas=1,
                    ))
        ...
        if self.use_js:
            ...
            sub = await self.js.subscribe(
                ...
                config=jsapi.ConsumerConfig(
                    ...
                    deliver_policy=jsapi.DeliverPolicy.NEW,  # 시작 이후만
    ...
    async def _on_msg(self, subject: str, msg):
        try:
            ...
            if subject.startswith("md.tick.dex.") and which == "dexSwapL1":
                # subject: md.tick.dex.<EX>.<CHAIN>.<PAIR>.swap
                parts = subject.split(".")
    ...
    async def _h_tick(self, exchange: str, symbol: str, instrument: str, t, ts_ns: int, lat_ms: Optional[float]):
        ...
        if self.enable_csv and self.csv_dir:
            await _csv_write(
                ...
                {"timestamp": _iso_from_ns(ts_ns), "exchange": exchange, "symbol": symbol,
                "instrument": instrument, "bid": t.bid, "ask": t.ask, "mid": t.mid,
                "lat_ms": f"{lat_ms:.2f}" if lat_ms is not None else ""}
            )
        await self._emit({
            "kind": "tick", "exchange": exchange, "symbol": symbol, "instrument": instrument,
            "mid": t.mid, "bid": t.bid, "ask": t.ask, "ts_ns": ts_ns, "lat_ms": lat_ms,
        })

    async def _h_funding(self, exchange: str, symbol: str, f, ts_ns: int, lat_ms: Optional[float]):
        async with self._lock:
            self.funding[(exchange, symbol)] = Funding(
                f.mark_price, f.index_price, f.estimated_settle_price,
                f.funding_rate, f.next_funding_time, ts_ns, latency_ms=lat_ms
            )
        if self.enable_csv and self.csv_dir:
            await _csv_write(
                ...
                {"timestamp": _iso_from_ns(ts_ns), "exchange": exchange, "symbol": symbol,
                 "mark_price": f.mark_price, "index_price": f.index_price,
                 "est_settle_price": f.estimated_settle_price,
                 "funding_rate": f.funding_rate, "next_funding_time_ms": f.next_funding_time,
                 "lat_ms": f"{lat_ms:.2f}" if lat_ms is not None else ""}
            )
        await self._emit({
            "kind": "funding", "exchange": exchange, "symbol": symbol,
            "rate": f.funding_rate, "next_ms": f.next_funding_time,
            "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
    ...
    async def _h_fee(self, exchange: str, symbol: str, instrument: str, f, ts_ns: int, lat_ms: Optional[float]):
        ...
        if self.enable_csv and self.csv_dir:
            await _csv_write(
                ...
                {"timestamp": _iso_from_ns(ts_ns), "exchange": exchange, "symbol": symbol, "instrument": instrument,
                 "taker_bps": f"{(f.taker_rate*10000.0):.6f}", "maker_bps": f"{(f.maker_rate*10000.0):.6f}",
                 "lat_ms": f"{lat_ms:.2f}" if lat_ms is not None else ""}
            )
        await self._emit({
            "kind": "fee", "exchange": exchange, "symbol": symbol, "instrument": instrument,
            "maker_rate": f.maker_rate, "taker_rate": f.taker_rate, "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
    ...
    async def _h_volume(self, exchange: str, symbol: str, instrument: str, v, ts_ns: int, lat_ms: Optional[float]):
        ...
        if self.enable_csv and self.csv_dir:
            await _csv_write(
                ...
                {"timestamp": _iso_from_ns(ts_ns), "exchange": exchange, "symbol": symbol, "instrument": instrument,
                 "volume0": v.volume0, "volume1": v.volume1, "high": v.high, "low": v.low, "trades": v.trades,
                 "lat_ms": f"{lat_ms:.2f}" if lat_ms is not None else ""}
            )
        await self._emit({
            "kind": "volume", "exchange": exchange, "symbol": symbol, "instrument": instrument,
            "volume0": v.volume0, "volume1": v.volume1, "high": v.high, "low": v.low, "trades": v.trades,
            "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
    ...
    async def _h_dexswapl1(self, exchange: str, chain: str, pair: str, ds, ts_ns: int, lat_ms: Optional[float]):
        ...
        async with self._lock:
            self.dex_pairs[(exchange, chain, pair)] = DexPair(
                exchange=exchange, chain=chain, pair=pair,
                token0=token0, token1=token1,
                price01=price01, price10=price10,
                invert_for_quote=invert, ts_ns=ts_ns,
                latency_ms=lat_ms
            )
        if self.enable_csv and self.csv_dir:
            await _csv_write(
        ...
        # 1) dexSwapL1 원본 이벤트
        await self._emit({
            "kind": "dexSwapL1", "exchange": exchange, "pair": pair,
            "token0": token0, "token1": token1,
            "price": price_qb, "price01": price01, "price10": price10,
            "invert_for_quote": invert, "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
        # 2) swap 틱으로도 발행( get_latest_price(...,"swap") 와 동일)
        await self._emit({
            "kind": "tick", "exchange": exchange, "symbol": pair, "instrument": "swap",
            "mid": price_qb, "bid": price_qb, "ask": price_qb,
            "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
    ...
    async def _h_dex_as_tick(self, exchange: str, pair: str, ds, ts_ns: int, lat_ms: Optional[float]):
        ...
        price01 = float(ds.price01)
        price10 = float(ds.price10)
        price_qb = price10 if invert else price01  # ✅ 뒤 토큰이 base, invert면 price10

        # DEX는 bid/ask가 없으니 mid로 동일 세팅
        async with self._lock:
            self.ticks[(exchange, pair, "swap")] = Tick(
                bid=price_qb, ask=price_qb, mid=price_qb,
                ts_ns=ts_ns, instrument="swap", latency_ms=lat_ms
            )

    async def _h_slippage(self, exchange: str, symbol: str, sl, ts_ns: int, lat_ms: Optional[float]):
        # DEX: symbol 은 보통 PAIR (예: WBNBETH)
        async with self._lock:
            self.slip[(exchange, symbol)] = Slippage(sl.impact_bps01, sl.impact_bps10, sl.block_number, sl.tx_hash, ts_ns, latency_ms=lat_ms)
        if self.enable_csv and self.csv_dir:
            await _csv_write(
                os.path.join(self.csv_dir, "slippage.csv"),
                ["timestamp","exchange","pair","bps01","bps10","block","tx","lat_ms"],
                {"timestamp": _iso_from_ns(ts_ns), "exchange": exchange, "pair": symbol,
                 "bps01": f"{sl.impact_bps01:.8f}", "bps10": f"{sl.impact_bps10:.8f}",
                 "block": sl.block_number, "tx": sl.tx_hash,
                 "lat_ms": f"{lat_ms:.2f}" if lat_ms is not None else ""}
            )
        await self._emit({
            "kind": "slippage", "exchange": exchange, "pair": symbol,
            "bps01": sl.impact_bps01, "bps10": sl.impact_bps10, "block": sl.block_number, "tx": sl.tx_hash,
            "ts_ns": ts_ns, "lat_ms": lat_ms,
        })
    ...
    async def get_fee(self, symbol: str, exchange: str, instrument: str) -> Optional[dict]:
        ...
        return {"maker_rate": f.maker_rate, "taker_rate": f.taker_rate,
                "maker_bps": f.maker_rate*10000.0, "taker_bps": f.taker_rate*10000.0, "ts": f.ts_ns}
    ...
    async def get_volume(self, symbol: str, exchange: str, instrument: str) -> Optional[dict]:
        ...
        return {"volume0": v.volume0, "volume1": v.volume1,
                "high": v.high, "low": v.low, "trades": v.trades, "ts": v.ts_ns}

    async def get_slippage_latest(self, pair: str, exchange: str) -> Optional[dict]:
        s = self.slip.get((exchange, pair))
        if not s: return None
        return {"bps01": s.impact_bps01, "bps10": s.impact_bps10, "ts": s.ts_ns}
    ...
    async def get_latest_price_with_latency(self, symbol: str, exchange: str, instrument: str = "spot"):
        ...
        return {
            "mid": t.mid, "bid": t.bid, "ask": t.ask,
            "lat_ms": t.latency_ms, "ts_ns": t.ts_ns, "ts_iso": _iso_from_ns(t.ts_ns),
            "instrument": t.instrument, "exchange": exchange, "symbol": _norm_symbol(symbol),
        }
    ...
    async def get_dex_price_qb(self, pair: str, exchange: str, chain: str = "BSC"):
        ...
        async with self._lock:
            ...
            return {"price_qb": price_qb, "ts_ns": dp.ts_ns, "ts_iso": _iso_from_ns(dp.ts_ns), "lat_ms": dp.latency_ms}
    ...
    async def get_spot_usdt_from_dex(self, symbol_usdt: str, exchange: str, chain: str = "BSC", router_token: str = "WBNB"):
        ...
        rq = f"{router_token}USDT"   # WBNBUSDT
        rb = f"{router_token}{base}" # WBNBETH
        ...
        async with self._lock:
            ...
            p_rq = dp_rq.price10 if dp_rq.invert_for_quote else dp_rq.price01  # WBNB/USDT
            p_rb = dp_rb.price10 if dp_rb.invert_for_quote else dp_rb.price01  # WBNB/ETH

            if not p_rq or not p_rb:
                return None

            price = p_rq / p_rb               # ETH/USDT
            ts_ns = min(dp_rq.ts_ns, dp_rb.ts_ns)  # 조합 스냅샷의 "유효 시점"은 더 오래된 쪽
            lat_ms = max(dp_rq.latency_ms or 0.0, dp_rb.latency_ms or 0.0)

            return {
                "price": price,
                "ts_ns": ts_ns, "ts_iso": _iso_from_ns(ts_ns),
                "lat_ms": lat_ms,
                "components": {
                    rq: {"qb": p_rq, "ts_ns": dp_rq.ts_ns, "lat_ms": dp_rq.latency_ms},
                    rb: {"qb": p_rb, "ts_ns": dp_rb.ts_ns, "lat_ms": dp_rb.latency_ms},
                }
            }
# ========================= helper builders =========================
def _dc_copy(obj):
